...
import {
  getActiveQuests,
  getCompletedQuests,
} from "../../state/quests/questSelectors";
      ...
      <div>
        <Col xs={12} key={1}>
          ...
          {completedQuests.map((quest) => {
            return <QuestCard key={quest.name} quest={quest} />;
          })}
        </Col>
      </div>
    );
  ...
  render() {
    ...
    return (
      <div className="content">
        <Grid fluid={true}>
          {activeQuests.length > 0 && this.getActiveQuestsView(activeQuests)}
          <Row>
...
export default withAuthorization(authCondition)(
  connect(mapStateToProps, mapDispatchToProps)(Quests)
);
